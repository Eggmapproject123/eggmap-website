...
export default function PaySuccessPage() {
  ...
  const standIdFromQuery = searchParams.get("standId") || "";
  const [standId, setStandId] = useState(standIdFromQuery);

  useEffect(() => {
    if (standIdFromQuery) {
      setStandId(standIdFromQuery);
      return;
    }
  ...
  const canSubmit = useMemo(() => {
    if (!paymentIntentId || !standId) return false;
    if (tookLastEggs) return true;
    return selectedTypes.length > 0;
  }, [paymentIntentId, standId, tookLastEggs, selectedTypes]);
      ...
      <div style={{ maxWidth: "680px", margin: "0 auto" }}>
        ...
        >
          ...
          <p style={{ marginTop: "12px", color: "#3e6b64" }}>
            Thanks for supporting local egg stands. Your payment has been received.
          </p>
        </section>
